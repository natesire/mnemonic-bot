...
export class Anagram {
  ...
  loadDictionary(): string[] {
    ...
    try {
      content = fs.readFileSync(this.dictionaryFile, 'utf-8');
    } catch (error) {
  ...
  // probably will not need this since hashmap lookup is constant time
  shardDictionaryByLength(dictionary: string[]) {

  }

  // can use as part of algorithm, determine max possible anagrams of a word
  longestWord() {
  }
